...
pub struct Bip44DerivationPath {
    pub coin_type: u32, // BTC=0, ETH=60, XMR=128
    pub account: u32,
...
impl TryFrom<Bip44DerivationPath> for bitcoin::util::bip32::DerivationPath {
    ...
    fn try_from(path: Bip44DerivationPath) -> Result<bitcoin::util::bip32::DerivationPath> {
        ...
        let mut path_vec = vec![
            ...
            ChildNumber::from_hardened_idx(path.coin_type)?,
...
impl HDPrivKey {
    pub fn new(master_seed: &[u8]) -> Result<Self> {
        Ok(Self {
            ext_key: ExtendedPrivKey::new_master(Network::Bitcoin, master_seed)?,
        })
    }
    ...
    pub fn key_part(&self) -> Vec<u8> {
        self.ext_key.private_key.to_bytes()
    }
...
mod tests {
    ...
    fn test_account0() {
        ...
        let child_key = HDPrivKey::new(master_seed)
            .unwrap()
